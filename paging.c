...
int ReadPageTable(const char *filename, PageTable *pt)
{
	...
	fscanf(fp,"%*s %d", &pt->bits_for_page_number);
	fscanf(fp,"%*s %d", &pt->bits_for_offset);
	fscanf(fp,"%*s %d", &pt->page_table_length);
...
int Translate(int logical_addr, PageTable *pt)
{
	...
	int frame, offset, physical;

	frame = pt->frame_number[logical_addr >> pt->bits_for_offset];
	offset = logical_addr % pt->page_size;
	physical = (frame << pt->bits_for_offset)|offset;
